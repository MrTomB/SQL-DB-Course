...
WHERE dv_customer.first_name = cb_customers.first_name 
      AND
      dv_customer.last_name  = cb_customers.last_name
      AND
...
WHERE mg_customers.address_id = dv_address.address_id AND dv_address.district
	IN(SELECT dv_address.district
       FROM dv_address
...
SELECT dv_film.rating, count(*) as count
FROM dv_film
